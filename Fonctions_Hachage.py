# Implémentation de fonctions de hachage (MD5, SHA-1, SHA-256) en Python

import math

# Fonctions utilitaires
def left_rotate(x, n):
    """Rotation à gauche de x sur n bits."""
    x &= 0xffffffff
    return ((x << n) | (x >> (32 - n))) & 0xffffffff

def to_bytes(n, length):
    """Convertit un entier en bytes (big-endian)."""
    return n.to_bytes(length, byteorder='big')

def from_bytes(b):
    """Convertit des bytes en entier (big-endian)."""
    return int.from_bytes(b, byteorder='big')

# MD5
def md5(message):
    """Calcule le hachage MD5 d'un message."""
    # Constantes
    K = [int(2**32 * abs(math.sin(i + 1))) & 0xffffffff for i in range(64)]
    S = [7, 12, 17, 22] * 4 + [5, 9, 14, 20] * 4 + [4, 11, 16, 23] * 4 + [6, 10, 15, 21] * 4
    
    # Initialisation des registres
    h0, h1, h2, h3 = 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476
    
    # Pré-traitement : convertir en bytes et ajouter le padding
    if isinstance(message, str):
        message = message.encode()
    orig_len = len(message) * 8
    message += b'\x80'
    while (len(message) % 64) != 56:
        message += b'\x00'
    message += to_bytes(orig_len, 8)
    
    # Traiter par blocs de 64 bytes
    for i in range(0, len(message), 64):
        chunk = message[i:i+64]
        w = [from_bytes(chunk[j:j+4]) for j in range(0, 64, 4)]
        
        # Initialisation des variables pour ce bloc
        a, b, c, d = h0, h1, h2, h3
        
        # Boucle principale
        for j in range(64):
            if j < 16:
                f = (b & c) | (~b & d)
                g = j
            elif j < 32:
                f = (d & b) | (~d & c)
                g = (5 * j + 1) % 16
            elif j < 48:
                f = b ^ c ^ d
                g = (3 * j + 5) % 16
            else:
                f = c ^ (b | ~d)
                g = (7 * j) % 16
            temp = d
            d = c
            c = b
            b = b + left_rotate((a + f + K[j] + w[g]) & 0xffffffff, S[j])
            a = temp
        
        # Mettre à jour les registres
        h0 = (h0 + a) & 0xffffffff
        h1 = (h1 + b) & 0xffffffff
        h2 = (h2 + c) & 0xffffffff
        h3 = (h3 + d) & 0xffffffff
    
    # Combiner les registres pour le hachage final
    return to_bytes(h0, 4) + to_bytes(h1, 4) + to_bytes(h2, 4) + to_bytes(h3, 4)

# SHA-1
def sha1(message):
    """Calcule le hachage SHA-1 d'un message."""
    # Initialisation des registres
    h0, h1, h2, h3, h4 = 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0
    
    # Pré-traitement
    if isinstance(message, str):
        message = message.encode()
    orig_len = len(message) * 8
    message += b'\x80'
    while (len(message) % 64) != 56:
        message += b'\x00'
    message += to_bytes(orig_len, 8)
    
    # Traiter par blocs de 64 bytes
    for i in range(0, len(message), 64):
        chunk = message[i:i+64]
        w = [from_bytes(chunk[j:j+4]) for j in range(0, 64, 4)]
        for j in range(16, 80):
            w.append(left_rotate(w[j-3] ^ w[j-8] ^ w[j-14] ^ w[j-16], 1))
        
        # Initialisation des variables pour ce bloc
        a, b, c, d, e = h0, h1, h2, h3, h4
        
        # Boucle principale
        for j in range(80):
            if j < 20:
                f = (b & c) | (~b & d)
                k = 0x5a827999
            elif j < 40:
                f = b ^ c ^ d
                k = 0x6ed9eba1
            elif j < 60:
                f = (b & c) | (b & d) | (c & d)
                k = 0x8f1bbcdc
            else:
                f = b ^ c ^ d
                k = 0xca62c1d6
            temp = (left_rotate(a, 5) + f + e + k + w[j]) & 0xffffffff
            e = d
            d = c
            c = left_rotate(b, 30)
            b = a
            a = temp
        
        # Mettre à jour les registres
        h0 = (h0 + a) & 0xffffffff
        h1 = (h1 + b) & 0xffffffff
        h2 = (h2 + c) & 0xffffffff
        h3 = (h3 + d) & 0xffffffff
        h4 = (h4 + e) & 0xffffffff
    
    # Combiner les registres pour le hachage final
    return to_bytes(h0, 4) + to_bytes(h1, 4) + to_bytes(h2, 4) + to_bytes(h3, 4) + to_bytes(h4, 4)

# SHA-256
def sha256(message):
    """Calcule le hachage SHA-256 d'un message."""
    # Constantes
    K = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
        0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
        0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
        0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
        0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
        0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ]
    
    # Initialisation des registres
    h = [
        0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
        0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
    ]
    
    # Pré-traitement
    if isinstance(message, str):
        message = message.encode()
    orig_len = len(message) * 8
    message += b'\x80'
    while (len(message) % 64) != 56:
        message += b'\x00'
    message += to_bytes(orig_len, 8)
    
    # Traiter par blocs de 64 bytes
    for i in range(0, len(message), 64):
        chunk = message[i:i+64]
        w = [from_bytes(chunk[j:j+4]) for j in range(0, 64, 4)]
        # Étendre w à 64 éléments
        w.extend([0] * (64 - len(w)))
        for j in range(16, 64):
            s0 = (right_rotate(w[j-15], 7) ^ right_rotate(w[j-15], 18) ^ (w[j-15] >> 3))
            s1 = (right_rotate(w[j-2], 17) ^ right_rotate(w[j-2], 19) ^ (w[j-2] >> 10))
            w[j] = (w[j-16] + s0 + w[j-7] + s1) & 0xffffffff
        
        # Initialisation des variables pour ce bloc
        a, b, c, d, e, f, g, h_val = h
        
        # Boucle principale
        for j in range(64):
            s0 = (right_rotate(a, 2) ^ right_rotate(a, 13) ^ right_rotate(a, 22))
            maj = (a & b) ^ (a & c) ^ (b & c)
            t2 = (s0 + maj) & 0xffffffff
            s1 = (right_rotate(e, 6) ^ right_rotate(e, 11) ^ right_rotate(e, 25))
            ch = (e & f) ^ (~e & g)
            t1 = (h_val + s1 + ch + K[j] + w[j]) & 0xffffffff
            
            h_val = g
            g = f
            f = e
            e = (d + t1) & 0xffffffff
            d = c
            c = b
            b = a
            a = (t1 + t2) & 0xffffffff
        
        # Mettre à jour les registres
        h[0] = (h[0] + a) & 0xffffffff
        h[1] = (h[1] + b) & 0xffffffff
        h[2] = (h[2] + c) & 0xffffffff
        h[3] = (h[3] + d) & 0xffffffff
        h[4] = (h[4] + e) & 0xffffffff
        h[5] = (h[5] + f) & 0xffffffff
        h[6] = (h[6] + g) & 0xffffffff
        h[7] = (h[7] + h_val) & 0xffffffff
    
    # Combiner les registres pour le hachage final
    return b''.join(to_bytes(h_val, 4) for h_val in h)
def right_rotate(x, n):
    """Rotation à droite de x sur n bits."""
    x &= 0xffffffff
    return ((x >> n) | (x << (32 - n))) & 0xffffffff

# Exemple d'utilisation
if __name__ == "__main__":
    message = "Bonjour, Hash !"
    
    # Calculer MD5
    md5_hash = md5(message)
    print(f"MD5 : {md5_hash.hex()}")
    
    # Calculer SHA-1
    sha1_hash = sha1(message)
    print(f"SHA-1 : {sha1_hash.hex()}")
    
    # Calculer SHA-256
    sha256_hash = sha256(message)
    print(f"SHA-256 : {sha256_hash.hex()}")