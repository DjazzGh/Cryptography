...
# Simple S-Box for demonstration (Real DES has 8 different S-boxes)
...
def des_encrypt(plaintext, key):
    ...
    round_keys = generate_keys(binary_key[:48])  # Using only 48 bits from 64-bit key

    for i in range(16):
        new_right = xor(left, f_function(right, round_keys[i]))
        left, right = right, new_right

    combined = right + left  # Swap at the end
...
# Example usage:
plaintext = "0123456789ABCDEF"  # 64-bit plaintext (hex)
key = "133457799BBCDFF1"  # 64-bit key (hex)
